			...
			<div id="cmp">
				<div class="cmp-flex-container">
					<div class="cmp-flex-label">
						<div>Label</div>
						<svg v-if="true" width="20" height="10">
							<circle cx="10" cy="5" r="2" fill="red" />
						</svg>
					</div>
					<div class="cmp-flex-state">{{ componentState }}</div>
					<btn v-if="true" class="cmp-flex-button">
						<svg
                ...
                <div>
                    <ul>
                        <li v-for="i in errorMessages">{{i}}</li>
                    </ul>
                </div>
			</div>
		</div>
    ...
    methods: {
        ...
        async checkConditions() {
            ...
            for(let i = 0; i < conditions.length; i++) {
                this.dataPointValues = await this.getDataPointValues(
                    this.getPointsToBeChecked(conditions[i])
                );
            ...
            condition.toChecked.some(c => {
                ...
                if(!!dp) {
                    ...
                    if(!conditionResult && !!c.toNote) {
                        this.errorMessages.push(`${condition.name} - ${c.describe}`);
                    }
                    ...
                    return false;
                    //Resturn error related to describe value
                }
            });
            if(conditionResult) {
        ...
        //Mamy warunki i przypisane do nich PD
        //Request zwraca Tablicę punktów z wartościami.
        //
