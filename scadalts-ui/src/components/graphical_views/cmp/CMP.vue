            ...
            <btn-group>
              <div v-for="item in controlsLevel0" :key="item.xid">
                ...
                >
                  {{ item.name }}
                  <span
                    v-if="item.runDirectlyBeforeShowSubMenu"
                    class="cmp-small-info"
...
export default {
  ...
  methods: {
    ...
    checkToNotificationError() {
      if (
        ...
        this.errorsNotification !=
          this.errorResultingFromOperationControl.length > 0
      ) {
        this.errorsNotification =
          this.newErrors.length > 0 ||
          this.errorResultingFromOperationControl.length > 0;
      }
    ...
    setActionLeve0(action) {
      ...
      if (runDirectlyBeforeShowSubMenu == true) {
        ...
        for (let i = 0; i < toSave.length; i++) {
          let xid = _.findWhere(
            this.config.control.definitionPointToSaveValue,
    ...
    checkIfThereAreSharesToConfirm() {
      ...
      if (
        typeof this.controlsLevel1 === "object" &&
        this.controlsLevel1.length > 0
      ) {
        result = true;
      } else {
  ...
  watch: {
    ...
    counterForAnaliseInOrder: function (val, oldVal) {
      if (this.counterForAnaliseInOrder >= 0) {
        ...
        let points = this.config.state.analiseInOrder[
          this.counterForAnaliseInOrder
        ].toChecked;
        let name = this.config.state.analiseInOrder[
          this.counterForAnaliseInOrder
        ].name;
