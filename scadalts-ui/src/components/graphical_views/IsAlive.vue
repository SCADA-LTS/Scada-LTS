...
<template>
    ...
    <div>
        <alert v-if="danger" id="p_is_alive_bdanger" type="danger"><b>Error</b> - time of the latest server response:
            {{timeFromServerEpoch | moment}}
        </alert>
        <alert v-if="warning" id="p_is_alive_bwarning" type="warning"><b>Warning</b> - time of the latest server
            response:
            {{timeFromServerEpoch | moment}}
        </alert>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      check () {
        this.$store.dispatch('isAlive').then(() => {
          this.setData()
        }).catch( () => {
          this.setData()
        })
    ...
    created () {
      ...
      this.$store.dispatch('setInitIsAlive', { tw: this.timeWarningEpoch, te: this.timeErrorEpoch, tr: this.timeRefresh})
    },
    mounted () {
      setInterval(
        function () {
