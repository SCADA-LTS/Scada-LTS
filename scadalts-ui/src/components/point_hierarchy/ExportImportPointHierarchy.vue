  ...
  methods: {
        ...
        axios
          ...
          .then((response) => {
            if (response.data !== undefined) {
              ...
              this.toCreate.xidFolderNotExists.push(
                this.base.xidFolderToCheck[0]
              );
            }
        ...
        for (var id in this.toCreate.xidFolderNotExists) {
          if (this.toCreate.xidFolderNotExists[id] != null) {
            ...
            if (validate) {
              ...
              this.toCreate.xidFolderToCreate.push(
                this.toCreate.xidFolderNotExists[id]
              );
            }
          }
        }
    ...
    prepareDataToMoveFolders() {
      ...
      for (var i in this.base.xidFolderExists) {
        for (var j in this.base.xidFolderBefore) {
          if (
            this.base.xidFolderExists[i].xidFolder ==
            this.base.xidFolderBefore[j].xid
          ) {
            this.showStatus(
              this.constants.STATUS_DETAIL,
              0,
              ...
              i + j,
              this.base.xidFolderExists.length +
                this.base.xidFolderBefore.length
            );
            if (
              ...
              let newMoveFolder = {};
              newMoveFolder.newParentXid = this.base.xidFolderExists[
                i
              ].parentXid;
    ...
    prepareDataToMovePoints() {
      ...
      for (var p in this.toMovePoints.xidMapPointsAfter) {
        ...
        let xidPointInCreateFolder = this.toMovePoints
          .xidMapPointsInCreateFolder[p];
      ...
      for (var p in this.toMovePoints.xidMapPointsExist) {
        ...
        if (xidPointInCreateFolder == undefined) {
          if (xidPointToCheckAfter == undefined) {
            ...
            xidPointToCheckAfter.xidFolder !== xidPointToCheckExist.xidFolder
          ) {
      ...
      for (let z in this.base.xidFolderAfter) {
        if (
          ...
          for (let g in this.base.xidErrors) {
            isNotInError = !(
              this.base.xidErrors[g].xidFolder == xidFolderToDelete
            );
    ...
    prepareFolderToNameChange() {
      ...
      for (var i in this.base.xidFolderAfter) {
        for (var j in this.base.xidFolderBefore) {
          if (
            this.base.xidFolderAfter[i].xidFolder ==
            this.base.xidFolderBefore[j].xid
          ) {
            if (
              this.base.xidFolderAfter[i].name !=
              this.base.xidFolderBefore[j].name
            ) {
    ...
    movePoints() {
      ...
      let arrPointToMove = Object.keys(
        this.toMovePoints.xidPointsMoveFromTo
      ).map((key) => this.toMovePoints.xidPointsMoveFromTo[key]);
      if (arrPointToMove.length > 0) {
        ...
        axios
          ...
          .then((response) => {
            ...
            delete this.toMovePoints.xidPointsMoveFromTo[
              arrPointToMove[0].xidPoint
            ];
    ...
    refreshCache() {
      ...
      axios
        ...
        .then((response) => {
          this.timer.stopImport = performance.now();
          this.timer.timeImport =
            this.timer.stopImport - this.timer.startImport;
          reload();
        })
        .catch((error) => {
          console.log(error);
