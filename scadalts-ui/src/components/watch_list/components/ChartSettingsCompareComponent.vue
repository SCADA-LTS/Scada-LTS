  ...
  <div class="col-xs-12">
    <div class="col-xs-12">
      <div class="col-xs-3">
        <select
          id="comp-el-1"
          v-model="pointSettings[0].dataPoint"
          class="form-control"
        >
        ...
        <div class="col-xs-6">
          ...
          <dropdown class="form-group row">
            ...
            <template slot="dropdown">
              <li style="padding: 10px">
                <time-picker
                  v-model="pointSettings[0].endTime"
                  :show-meridian="false"
                />
              </li>
            </template>
    ...
    <div class="col-xs-12">
      <div class="col-xs-3">
        <select
          id="comp-el-2"
          v-model="pointSettings[1].dataPoint"
          class="form-control"
        >
          <option v-for="p in pointList" v-bind:value="p" v-bind:key="p.id">
            {{ p.name }}: {{ p.xid }}
        ...
        <div class="col-xs-6">
          ...
          <dropdown class="form-group row">
            ...
            <template slot="dropdown">
              <li style="padding: 10px">
                <time-picker
                  v-model="pointSettings[1].endTime"
                  :show-meridian="false"
                />
              </li>
            </template>
  ...
  methods: {
    ...
    loadSettings() {
      let loadedData = JSON.parse(
        localStorage.getItem(`MWL_${this.watchListName}_P`)
      );
      if (!!loadedData) {
        if (loadedData.type === this.CHART_TYPE) {
          for (let i = 0; i < loadedData.pointSettings.length; i++) {
            this.pointSettings[i].dataPoint =
              loadedData.pointSettings[i].dataPoint;
            ...
            this.pointSettings[i].endDate = new Date(
              loadedData.pointSettings[i].endDate
            );
            this.pointSettings[i].endTime = new Date(
              loadedData.pointSettings[i].endTime
            );
          }
        }
      }
