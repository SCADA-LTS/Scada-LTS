  ...
  methods: {
    ...
    async fetchDataPoints(item) {
      ...
      dp.forEach((e) => {
        ...
        i.mail.push(
          this.addConfiguration(e.id, this.activeMailingList, "mail")
        );
        i.mail.push(
          this.addConfiguration(e.id, this.activeMailingList2, "mail")
        );
    ...
    addConfiguration(datapointId, mailingListId, type) {
      ...
      if (!!mailingListId) {
        if (type === "mail") {
          eventHandlerId = this.bindEmailEventHandler(
            datapointId,
            mailingListId
          );
        } else if (type === "sms") {
