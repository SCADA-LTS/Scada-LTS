...
<span v-if="!!textRenderer">
    <span v-if="textRenderer.type === 'textRendererPlain'">
        {{ pointRawValue}} <span v-if="!!textRenderer.suffix">{{textRenderer.suffix}}</span>
    </span>
    <span v-if="textRenderer.type === 'textRendererAnalog'">
        {{analogValue}}
    </span>
    <span v-if="textRenderer.type === 'textRendererRange'" :style="{color: activeColor}">
        {{rangeValue}}
    </span>
    <span v-if="textRenderer.type === 'textRendererBinary'" :style="{color: activeColor}">
        {{binaryValue}}
    </span>
    <span v-if="textRenderer.type === 'textRendererMultistate'" :style="{color: activeColor}">
        {{multistateValue}}
    </span>
...
export default {
    ...
    computed: {
        ...
        binaryValue() {
            const v = this.pointRawValue === "true" || this.pointRawValue === 1 || this.pointRawValue === true;
            if(v) {
