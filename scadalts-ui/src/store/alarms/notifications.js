...
const storeAlarmsNotifications = {
  ...
  actions: {
    ...
    deleteEventHandler({ dispatch }, eventHandlerId) {
      return dispatch(
        "requestDelete",
        `/eventHandler/delete/id/${eventHandlerId}`
      );
    },
    ...
    async createEventHandler({ state, dispatch }, payload) {
      let pedId = await dispatch(
        "createPointEventDetector",
        payload.datapointId
      );
      let edId = pedId.id;
      ...
      state.ehTemplate.xid =
        "EH_" + state.ehTemplate.xid + `_${dpId}_${edId}_${mlId}`;
      state.ehTemplate.alias =
        state.ehTemplate.alias + `_${dpId}_${edId}_${mlId}`;
