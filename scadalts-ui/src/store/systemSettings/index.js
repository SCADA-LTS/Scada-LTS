  ...
  actions: {
    ...
    saveDatabaseType(context, databaseType) {
      return new Promise((resolve, reject) => {
        axios
          .get(
            `${context.state.systemSettingsApiUrl}/saveDatabaseType/${databaseType}`
          )
          .then((response) => {
            if (response.status == 200) {
              resolve(true);
        ...
        axios
          ...
          .then((response) => {
            if (response.status == 200) {
              ...
              context.commit(
                "setSystemEventTypes",
                response.data.systemEventTypes
              );
              context.commit(
                "setAuditEventTypes",
                response.data.auditEventTypes
              );
              context.commit(
                "setSystemInfoSettings",
                response.data.systemInfoSettings
              );
        ...
        axios
          ...
          .then((response) => {
            if (response.status === 200) {
              ...
              context.commit(
                "setDefaultLoggingType",
                response.data.defaultLoggingType
              );
