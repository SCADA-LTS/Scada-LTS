...
export default new Vuex.Store({
  ...
  state: {
    ...
    requestConfig: {timeout: 5000, useCredentials: true, credentials: 'same-origin'},
  ...
  actions: {
    ...
    requestGet({state}, requestUrl) {
      return new Promise((resolve, reject) => {
        axios.get(state.applicationUrl + requestUrl, state.requestConfig).then(r => {
          if(r.status === 200) {
            resolve(r.data)
          } else {
            reject(false)
    ...
    requestPost({state}, payload) {
      return new Promise((resolve, reject) => {
        axios.post(state.applicationUrl + payload.url, payload.data, state.requestConfig).then(r => {
    ...
    requestDelete({state}, requestUrl) {
      return new Promise((resolve, reject) => {
        axios.delete(state.applicationUrl + requestUrl, state.requestConfig).then(r => {
          if(r.status === 200) {
            resolve(r.data)
          } else {
            reject(false)
    ...
    requestPut({state}, payload) {
      return new Promise((resolve, reject) => {
        axios.put(state.applicationUrl + payload.url, payload.data, state.requestConfig).then(r => {
          if(r.status === 201 || r.status === 200) {
            resolve(r.data);
          } else {
            reject(false);
