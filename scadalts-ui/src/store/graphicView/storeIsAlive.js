  ...
  actions: {
    calculateStateIsAlive({commit, state}, timeFromServer) {
      return new Promise((resolve) => {
        ...
        if (state.danger) {
          ...
          state.warning = ((state.timeInWebEpoch - state.timeServerEpoch) * 1000) >= state.timeWarningEpoch
          if (state.warning) {
            state.danger = false
            state.success = false
          } else {
    ...
    isAlive({state,commit,dispatch}) {
      return new Promise((resolve, reject) => {
          axios.get(`./api/is_alive/time`, {timeout: state.timeRefreshEpoch}).then(res => {
