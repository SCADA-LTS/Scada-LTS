...
public class PointLinksDwr extends BaseDwr {
    public Map<String, Object> init() {
        ...
        for (PointLinkVO pointLink : DaoInstances.getPointLinkDao().getPointLinks()) {
    ...
    public PointLinkVO getPointLink(int id) {
        ...
        if (id == Common.NEW_ID) {
            vo = new PointLinkVO();
            vo.setXid(DaoInstances.getPointLinkDao().generateUniqueXid());
        }
        else
            vo = DaoInstances.getPointLinkDao().getPointLink(id);
        return vo;
    }
    ...
    public DwrResponseI18n savePointLink(int id, String xid, int sourcePointId, int targetPointId, String script,
        ...
        else if (!DaoInstances.getPointLinkDao().isXidUnique(xid, id))
    ...
    public DwrResponseI18n validateScript(String script, int sourcePointId, int targetPointId) {
        ...
        if (point == null)
            ...
            int targetDataType = DaoInstances.getDataPointDao().getDataPoint(targetPointId).getPointLocator().getDataTypeId();
