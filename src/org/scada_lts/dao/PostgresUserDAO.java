...
import static org.scada_lts.dao.DAO.boolToChar;
import static org.scada_lts.dao.DAO.charToBool;

public class PostgresUserDAO implements IUserDAO {

	private static final Log LOG = LogFactory.getLog(PostgresUserDAO.class);
	...
	private static final String USER_SELECT_WHERE_USERNAME = ""
			+ USER_SELECT
			+ "where " + COLUMN_NAME_USERNAME + " ILIKE ?";
	...
	private class UserRowMapper implements RowMapper<User> {
		...
		public User mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			user.setAdmin(charToBool(rs.getString(COLUMN_NAME_ADMIN)));
			user.setDisabled(charToBool(rs.getString(COLUMN_NAME_DISABLED)));
			...
			user.setReceiveOwnAuditEvents(charToBool(rs.getString(COLUMN_NAME_RECEIVE_OWN_AUDIT_EVENTS)));
	...
	public List<User> getActiveUsers() {
		...
		return DAO.getInstance().getJdbcTemp().query(USER_SELECT_ACTIVE, new Object[]{boolToChar(false)}, new UserRowMapper());
	}
	...
	public int insert(final User user) {
		...
		DAO.getInstance().getJdbcTemp().update(connection -> {
			PreparedStatement ps = connection.prepareStatement(USER_INSERT, Statement.RETURN_GENERATED_KEYS);
