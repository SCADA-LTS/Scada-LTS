...
public class PostgresViewDAO implements IViewDAO {

    private Log LOG = LogFactory.getLog(PostgresViewDAO.class);
    ...
    private static final String VIEW_USER_INSERT_ON_DUPLICATE_KEY_UPDATE_ACCESS_TYPE = ""
            ...
            + COLUMN_NAME_MVU_ACCESS_TYPE + ") "
            + "values (?,?,?) "
            + "on conflict (" + COLUMN_NAME_MVU_VIEW_ID + ", " + COLUMN_NAME_MVU_USER_ID + ") "
            + "do update set "
            + COLUMN_NAME_MVU_ACCESS_TYPE + " = excluded." + COLUMN_NAME_MVU_ACCESS_TYPE;
    ...
    class ViewRowMapper implements RowMapper<View> {
        public View mapRow(ResultSet rs, int rowNum) throws SQLException {
            View v;
            InputStream inputStream = rs.getBinaryStream(COLUMN_NAME_DATA);
            if (inputStream != null) {
                v = (View) new SerializationData().readObject(inputStream);
            } else {
                // fallback, np. podczas aktualizacji struktury
                v = new View();
            }
    ...
    public void update(View entity) {
        DAO.getInstance().getJdbcTemp().update(
                con -> {
                    PreparedStatement ps = con.prepareStatement(VIEW_UPDATE);
                    ps.setString(1, entity.getXid());
                    ps.setString(2, entity.getName());
                    ps.setString(3, entity.getBackgroundFilename());
                    ps.setInt(4, entity.getAnonymousAccess());

                    ByteArrayInputStream dataStream = new SerializationData().writeObject(entity);
                    ps.setBinaryStream(5, dataStream);

                    ps.setInt(6, entity.getHeight());
                    ps.setInt(7, entity.getWidth());
    ...
    public View save(View entity) {
        ...
        DAO.getInstance().getJdbcTemp().update(new PreparedStatementCreator() {
            ...
            public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                ...
                ps.setInt(5, entity.getAnonymousAccess());

                ByteArrayInputStream bais = new SerializationData().writeObject(entity);
                ps.setBinaryStream(6, bais, bais.available());

                ps.setInt(7, entity.getHeight());
                ps.setInt(8, entity.getWidth());
        ...
        if (keys != null && keys.containsKey("id")) {
            ...
            throw new IllegalStateException("Missing 'id' key returned after inserting View");
        }
