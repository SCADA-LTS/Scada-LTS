...
public class V2_7_3_0__SqlDataSourceUpdate extends BaseJavaMigration {

    private static final Log LOG = LogFactory.getLog(V2_7_3_0__SqlDataSourceUpdate.class);
    ...
    private void migrate(JdbcTemplate jdbcTmp) {
        List<SqlDataSourceVO> dataSources = jdbcTmp.query("SELECT id, data FROM dataSources where dataSourceType = 6", (resultSet, i) -> {
            try (InputStream inputStream = resultSet.getBinaryStream("data");
                ...
                if(StringUtils.isEmpty(dataSourceVO.getJndiResourceName())) {
                    dataSourceVO.setJndiResource(false);
                }
                return dataSourceVO;
