...
--%><c:if test="${event.userNotified}">
  <c:choose>
    ...
    <c:otherwise>
      ...
      <c:if test="${event.active and not event.assignee}"><tag:img png="user_add" id="assigneeImg${event.id}" onclick="assignEvent(${event.id})" title="events.assign" style="display:inline;"/></c:if>
      <c:if test="${event.active and event.assignee}"><tag:img png="user_delete" id="unassigneeImg${event.id}" onclick="unassignEvent(${event.id})" title="events.unassign" style="display:inline;"/></c:if>
      <c:choose>
