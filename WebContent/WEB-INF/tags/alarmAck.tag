...
--%><c:if test="${event.userNotified}">
  <c:choose>
    ...
    <c:otherwise>
      ...
      <c:if test="${event.active and not event.accepted}"><tag:img png="tick" id="acceptImg${event.id}" onclick="acceptEvent(${event.id})" title="events.accepted" style="display:inline;"/></c:if>
      <c:choose>
