...
<script>
  ...
  export default {
      ...
      if (this.base.xidFolderToCheck.length == 0) {
        ...
        axios.get(apiCheckXidFolder).then(response => {
          if (response.data !== undefined) {
            ...
            if (this.base.xidFolderToCheck[0].pointXids.length > 0) {
              this.toCreateAndMove.xidFolderToCreateAndPointsMove.push(this.base.xidFolderToCheck[0]);
            } else {
      ...
      if (this.toCreate.xidFolderNotExists.length > 0) {
        for (var id in this.toCreate.xidFolderNotExists) {
          if (this.toCreate.xidFolderNotExists[id] != null) {
            ...
            console.log("ERROR - I don't create :" + JSON.stringify(this.toCreate.xidFolderNotExists[id]));
      ...
      if (this.toCreate.xidFolderToCreate.length > 0) {
        ...
        axios.post(apiAddFolder, data)
          .then(response => {
            ...
            this.moveArrPoints(newCreated.pointXids, response.data.xidFolder, newCreated.pointXids.length);
    ...
    moveArrPoints(arrPointsToMove, xidFolder, callback, count) {
      if (this.canceled()) return;

      this.showStatus(
        this.constants.STATUS_GROUP,
        this.constants.STATUS_GROUP_INFO.MOVE_POINTS,
        "Move points for:" + xidFolder);

      if (arrPointsToMove.length > 0) {
        this.showStatus(
          this.constants.STATUS_DETAIL,
          0,
          "xid:" + arrPointsToMove[0].xidPoint,
          count,
          count - arrPointsToMove.length
        );

        const apiMovePoints = `./api/pointHierarchy/pointMoveTo/${arrPointsToMove[0].xidPoint}/${xidFolder}`;
        axios.put(apiMovePoints)
          .then(response => {
            console.log(response);
            let moved = arrPointsToMove[0];
            this.toMovePoints.xidPointMoved.push(moved);

            arrPointsToMove.splice(0, 1);
            if (arrPointsToMove.length > 0) {
              this.moveArrPoints(arrPointsToMove, xidFolder, count);
            } else {
              callback();
            }
          })
